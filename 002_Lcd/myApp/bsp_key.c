...
//			 LOG_INFO("---> key%d press down! <---\r\n",btn->button_id);
            switch (_btn->button_id) {
                case btn1_id:
                    ...
                    if (current_index > 0) current_index--;  // 当前索引减一
                    else current_index = (current_menu->num_children - 1); // 滚回到最后一个
                    break;
                case btn2_id:
                    led_setup(2, 40, 0.5f, 3);
                    buzzer_setup(50, 0.3f, 1);
                    if (current_index < (current_menu->num_children - 1)) current_index++;  // 当前索引加一
                    else current_index = 0; // 滚回到第一个
                    break;
                case btn3_id:
                    led_setup(4, 40, 0.5f, 3);
                    buzzer_setup(50, 0.5f, 1);
                    if (current_menu->children[current_index].children != NULL) {
                        // 进入子菜单
                        current_menu = &current_menu->children[current_index];
                        current_index = 0;
                    }
                    break;
                case btn4_id:
                    led_setup(6, 40, 0.5f, 3);
                    buzzer_setup(50, 0.7f, 1);
